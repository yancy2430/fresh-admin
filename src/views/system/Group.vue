...
<script>
import {list as groupList, getById, submitGroup, removeGroupById} from '@/api/SysGroup'
import {toTree} from "@/utils/util";
import {message} from 'ant-design-vue';
import {list as resourcesList} from '@/api/SysResources'
...
export default {
  ...
  methods: {
    ...
    confirmDel() {
      removeGroupById({groupId: this.itemForm.id})
          .then(res => {
            if (res.code === 200) {
    ...
    saveHandleOk() {
      ...
      submitGroup(this.itemForm).then(res => {
        if (res.code === 200) {
