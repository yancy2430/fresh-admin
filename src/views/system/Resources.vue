...
<script>
import {list as resourcesList, methods,getById,save as resourcesSave,removeById as resourcesRemoveById,updateById as resourcesUpdateById} from '@/api/SysResources'
...
export default {
  ...
  methods:{
    ...
    confirmDel(){
      resourcesRemoveById([this.selectTreeValue]).then(res=>{
        if (res.code!==200){
    ...
    confirmDels(){
      resourcesRemoveById(this.checkedKeys).then(res=>{
        if (res.code!==200){
          message.error(res.msg);
        }else {
          message.success('删除成功');
    ...
    submitHandleOk(){
            ...
            resourcesUpdateById(this.treeValue).then(res=>{
              if (res.code!==200){
                message.error(res.msg);
              }else {
